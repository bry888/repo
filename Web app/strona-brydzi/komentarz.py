...
class AddComHandler(webapp2.RequestHandler):
	def write_form(self, error2="", nick="", comment=""):
		...
		self.response.out.write(form3 % {"error2": error2, "nick": escape_html(nick), "comment": escape_html(comment), "comments": escape_html(comments)})
	def get(self):
		self.write_form()
	def post(self):
		...
		if nick and comment:
			...
			write_form("Wypelnij pola!", nick, comment)
		
class CommentsHandler(webapp2.RequestHandler):
	def get(self):
		self.response.out.write(form4)
		nick = self.request.get('nick')
		comment
